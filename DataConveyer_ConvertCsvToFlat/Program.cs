...
namespace DataConveyer_ConvertCsvToFlat
{
   internal class Program
   {
      ...
      static void Main()
      {
         ...
         Console.WriteLine();
         Console.WriteLine("This application translates a list of aircraft from CSV to a flat file format.");
         Console.WriteLine();
      ...
      private static async void FileDroppedHandler(object sender, FileSystemEventArgs e)
      {
         ...
         {
            Console.WriteLine($"Processing {fname} file completed in {stopWatch.Elapsed.TotalSeconds.ToString("##0.000")}s; {result.ClustersWritten} out of {result.ClustersRead} plane records posted onto output.");
         }
